...
<script setup>
    ...
    const randomGetwayPost = async () => {
        let newPath;
        if (NextEchoStore.application.actives.length === 1) return await ElMessage({type: 'warning', message: "当前只有一篇文章哦！"});
        do {
            const lenround = randomPost();
            newPath = NextEchoStore.application.actives[lenround].config.path;
    ...
    const randomPost = () => { const lengths = NextEchoStore.application.actives.length - 1; return utils.getRandomIntInclusiveInter(0, lengths) };
</script>

<template>
    <span class="title-span">
        <a class="span-link" href="/">{{ NextEchoStore.configGlobal.site_title }}</a>
    </span>

    <el-button class="mobile-left-button mobile-button" :icon="Menu" @click="ReomEchoStore.setMobileDrawerStat(true)" v-if="ReomEchoStore.isDeviceMobilePhone && NextEchoStore.configDrawer.drawer_show"/>

    <span class="right-span">
        <span class="button-group">
            <el-tooltip content="切换主题" placement="bottom" effect="light" v-if="NextEchoStore.configHeader.theme_widget">
            ...
            <el-tooltip content="放大/缩小" placement="bottom" effect="light" v-if="NextEchoStore.configHeader.resize_widget">
            ...
            <t-popup trigger="hover" v-if="NextEchoStore.configHeader.music_widget">
            ...
            <el-tooltip content="随机开往" placement="bottom" effect="light" v-if="NextEchoStore.configHeader.rando_widget">
            ...
            <el-tooltip content="界面设置" placement="bottom" effect="light" v-if="NextEchoStore.configHeader.header_setting">
            ...
            <el-tooltip content="搜索组件（Ctrl + K）" placement="bottom" effect="light" v-if="NextEchoStore.configHeader.search_widget">
            ...
            <el-tooltip content="用户中心" placement="bottom" effect="light" v-if="NextEchoStore.configHeader.users_widget">
