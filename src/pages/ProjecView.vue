...
import { useNextEchoStore } from "@/stores/NextEchoStore"

    const onReady = ref(true);
    const content = ref(null);
    const ReomEchoStore = utils.useReomEchoStore(),NextEchoStore = useNextEchoStore();
    const filePath = 'https://api.github.com/users/' + NextEchoStore.configAuthor.author_github + '/repos';
...
<template>
    <div class="project-list" v-if="Array.isArray(content)">
        <div class="item-container" v-for="(item,index) in content" :key="index">
            <div class="project-item" v-wave="NextEchoStore.configSundry.moused_waves && !ReomEchoStore.isDeviceMobilePhone" :style="{ backgroundColor: utils.returnRoundColor() }">
