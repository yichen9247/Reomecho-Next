...
<script setup>
    ...
    import { circleInit } from '@/stores/NextEchoStore'
    ...
    onMounted(async () => {
        if (NextEchoStore.configCircle === null) await circleInit();
        await axiostool.sendHttpGet(NextEchoStore.configCircle.serverAdress + '/all').then(async (data) => {
        ...
        postIndex.value = NextEchoStore.configCircle.eachItem;
        postArray.value = list.value.reverse().slice(0,postIndex.value);
...
<template>
    ...
    <ul class="circle-list" v-if="NextEchoStore.configCircle !== null && postArray.length > 0">
    ...
    <NotDataView description="暂无更多数据惹" v-if="NextEchoStore.configCircle !== null && postArray.length === 0" style="border: none" />

    <div class="load-more-circle" v-if="NextEchoStore.configCircle !== null && postArray.length > 0">
