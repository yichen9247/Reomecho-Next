...
<script setup>
    ...
    const router = useRouter(),Cookies = utils.Cookies,ReomEchoStore = utils.useReomEchoStore(),NextEchoStore = utils.useNextEchoStore();
...
<template>
    <el-drawer v-model="ReomEchoStore.mobileDrawerStat" :show-close="false" :destroy-on-close="false" :with-header="false" direction="ltr" size="80%" class="mobile-side-bar" v-if="NextEchoStore.configDrawer.drawer_show && ReomEchoStore.isDeviceMobilePhone" @open="checkSiteHref">
        <div class="sidebar">
            ...
            <img class="side-out-image" v-lazy="NextEchoStore.configDrawer.drawer_image" draggable="false">
            <div class="side-out-autho drawer-card">
                <img class="avatar-image" v-lazy="NextEchoStore.configAuthor.author_avatar" :alt="NextEchoStore.configAuthor.author_name">
                <div class="author-infos">
                    <a class="author-link" :href="NextEchoStore.configAuthor.author_link">{{ NextEchoStore.configAuthor.author_name }}</a>
                    <p class="author-desc">{{ NextEchoStore.configAuthor.author_desc }}</p>
                </div>
            </div>

            <MobileSocials v-if="NextEchoStore.configDrawer.drawer_statistics"/>

            <div class="side-out-search drawer-card" v-if="NextEchoStore.configDrawer.drawer_search">
                <MobileSearch/>
            </div>
            <ul class="nav-list-mobile drawer-card">
                <li v-for="(item, index) in NextEchoStore.configHeader.header_navlist" :key="index" class="nav-item">
            ...
            <div class="side-out-music drawer-card" v-if="NextEchoStore.configDrawer.drawer_musicst">
                <StaticMusicCard :player="true"/>
            </div>

            <MobileStatistics v-if="NextEchoStore.configDrawer.drawer_statis"/>

            <div class="side-out-footer drawer-card" v-if="NextEchoStore.configDrawer.drawer_footer" v-wave="NextEchoStore.configSundry.moused_waves">
