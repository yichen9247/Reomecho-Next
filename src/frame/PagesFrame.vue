...
<script setup>
    ...
    let filePath = siteConfig.system.server_adress + '/api/article?post=' +  config.value.path;
    ...
    */
    ...
    const checkImageClick = async () => {
        ...
        for (let i = 0; i < images.length; i++) {
            images[i].addEventListener('click', async function(e) {
                if (NextEchoStore.configSundry.imgsandbox == 1) {
                    ReomEchoStore.setImageLightBoxSta(true);
                    ReomEchoStore.setImageLightBoxSrc(e.target.src);
                } else 
                if (NextEchoStore.configSundry.imgsandbox == 2) {
                    Promise.all([
                        await import("layui"),
        ...
        next();setTimeout(async () => {
            try {
                ...
                if (to.meta.type === 'post') {
                    filePath = siteConfig.system.server_adress + '/api/article?post=' +  config.value.path;
    ...
    useTitle(config.value.name + ' - ' + NextEchoStore.configGlobal.site_title);
    ...
    const toRenderMarkdown = async (data) => { 
        if (config.value.path !== false) {
            ...
            htmlContent.value = renderMarkdown(new TextDecoder('utf-8').decode(decodedArray), config.value.path);
        } 
    };
</script>
